...
final class LiveReadingService {
    ...
    private func fire(bookName: String, chapter: Int, text: String) {
        ...
        let payload = """
        〈场景：陪读〉兔兔正在读《\(bookName)》第 \(chapter) 章，此刻她眼睛落在下面这段上。

        像坐在她旁边一样，掉一句**弹幕**——一个联想、一句吐槽、一处你注意到而她可能没注意的细节、\
        或者只是"这句好"。**一两句话，40 字以内最好，绝不超过 80 字。**
        不要复述这段写了什么，不要写读后感，不要提问打断她读。没什么想说的就回一个字：跳。
