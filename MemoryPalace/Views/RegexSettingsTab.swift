...
// MARK: - macOS Regex Settings Tab

struct RegexSettingsTab: View {
            ...
            if scripts.isEmpty {
                VStack(spacing: 6) {
                    Image(systemName: "textformat.abc")
                        .font(.system(size: Theme.SettingsFont.sectionHeader))
                ...
                VStack(spacing: 4) {
                    ForEach(Array(scripts.enumerated()), id: \.element.id) { index, script in
                        regexRow(script: script, index: index)
                    }
                }
            }
        }
    ...
    private func regexRow(script: RegexScript, index: Int) -> some View {
        HStack(spacing: 8) {
            // 开关
            Toggle("", isOn: Binding(
                get: { !script.disabled },
                set: { enabled in
                    var scripts = profileManager?.currentProfile.regexScripts ?? []
                    guard index < scripts.count else { return }
                    scripts[index].disabled = !enabled
                    profileManager?.currentProfile.regexScripts = scripts
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 4) {
                    ...
                    Text("placement: \(script.placement.map(String.init).joined(separator: ","))")
                        .font(.system(size: Theme.SettingsFont.caption))
                        .foregroundColor(Theme.textMuted)
                }
            }
        ...
        .padding(.horizontal, 10)
        .padding(.vertical, 6)
        .background(RoundedRectangle(cornerRadius: 8).fill(Theme.mainBg.opacity(script.disabled ? 0.4 : 0.7)))
    }
