...
- [~] **P3-11 无缝上下文/跨窗口记忆**：✅ Phase 1（2026-07-12）——ContextInheritance：New chat 长按菜单「接着聊（继承上文）」，继承当前对话的压缩脉络（A）+ 最近 8 条原文（C）写成新对话初始前情提要（summaryLayer 注入，滞回压缩自然融合）；继承对话标题带 ↳ 前缀；深继承时跨窗口轻摘要让位。设计稿 docs/PLAN-SEAMLESS-CONTEXT.md。待办：B 层记忆库 RAG（Phase 2）、任选对话/设置开关（Phase 3）、CC 对齐（Phase 4）
