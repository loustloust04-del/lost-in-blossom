...
- [ ] **P1-1 Token/缓存命中率增强**：TokenStatsStore 已存 cacheRead/cacheWrite → 汇总卡 + 命中率趋势 + 按模型分组
- [ ] **P1-2 邮件双端收口**：API 侧 gmail_* 已通（网关 builtin）；验证 CC 侧通路（.mcp.json 或直调网关），双端实测各发一封
- [ ] **P1-3 低电量/定位提醒**：phone-status 已上报 battery/place → 网关规则引擎：电量 < 阈值 → APNs 提醒；进出常去地点 → 消息。规则做成可配置（复用 admin 层）
- [ ] **P1-4 CC/API 读写控制台**：网关 builtin 加 console_read / console_write（DailyContext：饮水/进食/药物/备注），双端可读可记
