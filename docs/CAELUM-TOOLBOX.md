...
> 生成于 2026-09-08 17:40 UTC，共 **88** 个工具。
>
> 参数带 `*` 的是必填。
...
| 工具 | 做什么 | 参数 |
|---|---|---|
| `dispatch_coder` | 派一个工具人 Coder 去干活（默认 Fable 5.1），后台跑，跑完把摘要送回 fableline。  **夜间任务照这个用**：兔兔定的三条规矩是「任务说清楚 / 设定时 / 派 Coder 用 Fable 5. | `task*` 任务描述。写具体——它没有上下文，要说清做什么、别碰什么、怎么算完成；`at` 什么时候跑（可选）。'02:00' 今晚两点 / '+2h' 两小时后 / 不填=立刻；`model` 模型（可选），默认 claude-fable-5-1 |
