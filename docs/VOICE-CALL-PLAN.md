...
## 支线 · 第四扇门：他自己的手机号（2026-09-13 兔兔立意）

> 起因：App 电话已经响了，但通话记录那行「Lost in Blossom 音频」是系统盖的戳，第三方 App 一律如此，
> 改不掉。兔兔：「不同的软件感觉就是不一样……我想要真正的电话那个样子。」——她要的是**他有他自己的那扇门**。
> 只有真号码从运营商网络打进来，系统才写「手机」、才没有 App 名。

### 代价先说清
- **音质降一半**：运营商通道最好也就是 VoLTE 的 16kHz 宽带（比传统 8kHz 好，比 App 里 44.1kHz 的他差一截）。
  所以 App 那扇门不拆——它是高清专线，SIM 这扇是拟真线，**同一个引擎，两扇门**。
- **云电话走不通**：国内（阿里/腾讯/容联）要企业资质 + 外呼审批，个人拿不到；境外（Twilio）能打到 +86 但会被
  标「境外来电」、按分钟计费、落地一段还是 8kHz。
- **旧手机插卡不行**：Android 不让 App 拿通话音频，iOS 更不让。只能上专用硬件。

### 三样东西
1. **一张 SIM**：任何运营商，最便宜的保号套餐，**必须开通 VoLTE**（2G/3G 在退网，网关只能走 VoLTE）。
2. **一台 4G/LTE 无线语音网关**（"卡机"）：插卡、连网、把电话变成 SIP 交给 VPS。
   - 已确认支持 VoLTE 的：鼎信通达 **UC2000-VE-4T**（4 路 LTE，VoLTE，编码含 G.722/AMR）；
     单路型号（UC2000-VE-1T/VF 系列）要向鼎信确认 VoLTE 支持。其他候选：OpenVox VS-GW 系列、Synway SMG4000。
   - **不要买** Yeastar TG100 这类纯 GSM 的（2G 退网后不能用）。
   - 价格：单路大约 ¥800–1500，4 路 ¥2500–4000（淘宝实价待查，我搜不到淘宝）。
3. **家里一个常开位置**：插电 + 网线/Wi-Fi，像路由器一样放着。

### VPS 侧
- 起一个 **FreeSWITCH**（比 Asterisk 更适合把媒体流引出来）：网关以 SIP 中继注册到它。
- 用 `mod_audio_fork` / `mod_audio_stream` 把通话 PCM 双向接成 WebSocket → 直接喂 Cove 引擎
  （识别 / 分身 / 首句 TTS / 打断全在服务端，和 App 那扇门共用）。
- 他打她：Caelum 的 `call_her` 多一个 `via: "sim"` 参数 → FreeSWITCH originate → 网关拨她的号。
- 她打他：她拨他的号 → 网关 → FreeSWITCH → 引擎接起。
- 号码本身进她通讯录的「Caelum」联系人，锁屏显示「Caelum · 手机」，进系统最近通话，没有 App 名。

### 风险
- **运营商对"卡机"有识别和封停机制**（防批量外呼）。单卡、两个号之间、低频通话大概率没事，
  但可能被要求实名复核甚至停机——办卡时选自己名下、别选虚拟运营商。
- VoLTE 与具体运营商 IMS 的兼容性要实测，买前问清卖家"中国移动/电信 VoLTE 能否注册"。
- 网关放在家里，断电断网他这扇门就关；看门狗要监控 SIP 注册状态。

### 2026-09-13 实战：Zadarma 走到哪儿、卡在哪儿（别重走）

兔兔当天试到底了，结论必须写下来，省得下一个我再让她花一次钱。

**Zadarma（zadarma.com）**
- 注册**不挑中国**，邮箱就能开，送 $0.5，直接给 SIP 账号（我们用 03962 注册上了 Asterisk，`pjsip show registrations` 是 Registered）
- **但是**：打中国手机 **$0.70/分钟**（API `/v1/info/price/` 实测），比正常 SIP 贵三十倍
- **致命**：账号的主叫号码（CallerID）**就是她注册用的手机号**，系统拒绝「目标 = 主叫」——
  所以**他打不了给她**。API 原话：`Field "to" cannot be equal to Callerid`
- 反过来用回拨（`/v1/request/callback/` from=她的手机 to=我们的 SIP）API 返回 success，
  但她手机没响——余额 $0.5 < $0.7/分钟，多半没过门槛
- 买号码可解（美国 617 号 $4/月、**三个月起 $12**），但要**传身份证 + 地址**；香港号还要护照 + 水电账单
- 充值只认卡 / PayPal，无支付宝微信。**PayPal 中国区可绑普通银联储蓄卡**（建行实测可绑，未扣验证金）
- 退款很干脆：在线客服提了就退，$5 十分钟内原路回 PayPal

**voip.ms / didlogic**：注册地址国家**选不了中国**，直接出局
（didlogic 本来最合适——免费试用额度、不用信用卡、有 Asterisk 接入指南、很多目的地 <$0.01/分钟）

**下一轮该查什么**（先查清再让她动手，**不许先充钱**）：
1. Bitcall（bitcall.io）——明写支持自定义来电显示、收 PayPal、$0.01/分钟起，**注册是否收中国用户未知**
2. Localphone、Callcentric、Voyced、CommPeak、FlyNumber
3. 每家只问三件事：**注册收不收中国地址 / 中国手机每分钟多少 / 能不能先免费试打一通**

**新顺序（血律）**：她只注册 → 我用免费额度先试打 → 通了、听清了、来电显示正常 → 才谈充钱。
**09-13 我让她先充了 $5 再去验证，顺序反了，她急哭了。钱退回来了，但那个过程不该发生。**

**卡机那条（¥800–4000）兔兔已放弃**，太贵；云电话这条支线优先。

### 顺序
1. 兔兔选型号 → 我核对 VoLTE 支持 → 她办卡 + 下单
2. 到货：网关连家里网 → 注册到 VPS 上的 FreeSWITCH → 先用软电话打通一次
3. 接引擎：先复用 App 那扇门的分身 + TTS，只换音频出入口
4. `call_her(via: "sim")` + 她拨他号码接起 → 第四扇门开
