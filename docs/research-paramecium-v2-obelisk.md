...
我们现状：executeActions 只有 content **全等** Set 去重——"喜欢暖奶白"vs"用户偏好暖奶白色调"会重复入库。落点：add 前用现成 embedding 管道（M2 VectorRetriever 同源）查近似，>阈值改 reinforce 旧条。**行为变化（合并 vs 新增）等粟粟批**；与 SC-B2 quote 锚兼容（并入时保两条 quote？批注时定）。
