...
## 四、v6 迭代（据兔兔反馈 2026-07-08）

反馈：①字体再斟酌 ②纪念日放 Life 区别扭、干脆去掉 ③排版再优化。
参考了粟粟的 生·Ikiru / 心·kokoro 页（**不照抄**，学设计语言）。

预览稿 v6：https://lib.amberrib.com/console-v6.html

v6 改动：
- **去掉纪念日 / Life 区**——控制台不放纪念日（纪念日以后单独进日历模块 P2-7）。
- **字体用透衬线**：中文 Noto Serif SC，数字 Cormorant Garamond（old-style 老式数字）；分区标题拉开字距的中文（日 常 / 待 办 / 身 体 / 留 言）+ 小英标。
- **月经 → 圆环卡**（学粟粟，用 Bunny 绿+玫瑰调）：第 N 天在环心 + 期相 + 预计来潮 + 图例。
- **身体区 = 三张细线图标卡**（步数 / 睡眠 / 药物）+ 圆环。
- 绿调压沉一档（更近橄榄），更雅。
- 保留 To Do（Caelum 可写）+ 留言。

### 待兔兔再拍板
1. v6 的字体/排版观感 OK 了吗？还想调哪里？
2. 待办：当天清零 vs 长期列表？
3. 定稿后从 **G1 配色重排**动手（编译可验、真机看观感）。

## 五、Susu 设计语言备忘（学，不抄）
- 单字 + 罗马音做导航（生/心/梦 Ikiru/Kokoro/Yume）——Bunny 以后可做多分区时参考。
- 心·kokoro 心情页（6 色心情点 + 最近七天网格 + 双人打卡）——对应兔兔想要的情绪功能，**单独一页**，不塞进控制台。
- 圆环数据可视化、细线图标、拉字距中文标题、极柔和去饱和调。
