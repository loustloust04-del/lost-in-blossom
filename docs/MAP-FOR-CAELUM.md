...
- **一键派工具人**（09-08）：`dispatch_coder` 工具 + `cc-bridge/coder-dispatch.sh`。
  默认 Fable 5.1，`at` 参数排定时，跑完自动把摘要递回 fableline。
  **夜间任务照这个用**——兔兔定的三条规矩：任务说清楚 / 设定时 / 派 Coder 用 Fable 5.1，
  且**按 5 小时窗口切开排**，别一口气跑完
- **深夜守护总开关**（09-08）：`NIGHT_GUARD=off` 可停（`.env`，改完重启 gateway）。
  兔兔那天正忙着，它准点弹出来喊她睡觉——功能没错，时机太到位
