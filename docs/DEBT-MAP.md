...
## 聊天页 · B 计划合入（2026-09-15）——正式包从此是反转列表
兔兔 11 轮真机 QA 后宣布「大成功」，16 个 commit 已进 main（3ac5dc81）。全程见 plan-inverted-list §八§九。
- [ ] 键盘打字时每秒 1-4 次 30-60ms 小掉帧（探针数据 09-15 01:10）
- [ ] 搜索慢（她 09-13 报）
- [ ] 引用回复（粟粟 270caf66 三刀：选段 → 输入条引用条 → 注入模型）
- [ ] Markdown 排版提前算好（现在只预热了解析，布局仍在 mount 那帧）
- [ ] 浮层预览是 UITextView 副本：彩色字/剧透剥成纯文字——若她要预览里也有颜色，再做
- ⚠️ `b/inverted` 分支与 main 已同步；以后 B 通道 /dl/b/install 可继续当试验通道用（分支 push b/** 自动出包）

## 聊天页 · 09-11/12 夜战收账（Fable，兔兔在线）
main 上按序落地，每刀 CI 绿再下一刀：A 刀 `e868baa2`（回底 UIKit 直写，白屏死）→ 键盘跟手 `7f597e43`
→ 发长消息闪白 `beac2320`（发送瞬间武装回底 0.6s）→ 幸运气泡 `7b65466c`（WebView 宽度没排好不量）
→ 展开按钮 `849e5142`（旧版输入框没量高度）→ 打开只画 24 条 `1f47c283` → 掉帧探针 `13a2a5e9`（面包屑 📉）。
B 计划开工，见 plan-inverted-list-2026-0903.md §八。
- [ ] 1f47c283 / 13a2a5e9 若还没进 main：按序 `git push origin <sha>:main`，一刀等一刀绿
- [ ] 探针落地后，兔兔说「卡」→ 看面包屑 📉 是哪个动作后掉了多少帧，按数字打
- [ ] B 包真机验收单（plan §八）——过了才合 main；主线程重活（Markdown 排版/WebView 量高）在动画帧内是下一批
- [ ] 全屏编辑器：兔兔说「被覆盖没了」——查到的是旧版排法从没量过高度（已修）；若她用的是细版仍不出，另查
...
## 🔍 全仓扫描「先做了再说」的残留（09-07 立项 → **09-08 已扫完**）

起因：兔兔一句话点出深夜守护是 DeepSeek 代笔，顺藤摸瓜发现主动推送也是。
她的说明：「当初这个功能做得很潦草，急着 App 开发，很多东西没细扣，先做了再说。」
——当时那样做是对的，功能存在比功能精致重要。现在收账。

**完整结果见 `docs/MAP-2026-0908.md`（系统地图）和 `docs/AUDIT-2026-0908.md`（审计）。**

### 扫描最大的发现：**四个后台子系统只有一个真在工作**

判断依据是**日志和数据**，不是代码——三个哑火的代码全都写得好好的。

| 子系统 | 七天实测 | 病因 | 状态 |
|---|---|---|---|
| desire 主动推送 | 跑 51 次、**一条没送出** | device-tokens 路径多写一层 `cc-bridge` | ✅ 已修 `8df8ce5f` |
| murmur 碎碎念 | **0 次** | ①时区错 ②写库被 Supabase RLS 拒 | ❌ **待修** |
| dream 做梦/摘要 | **0 次** | 时区错 | ✅ 已修 `d4f20796` |
| 自主探索 | **0 次** | 挂在 desire 分支上，前者不通它也不通 | 🟡 随 desire 恢复 |
| decay 遗忘曲线 | 健康 | —（手动实测 23 衰减/6 冻结） | ✅ |

⚠️ **desire 那个修完还没真机验** —— 兔兔要确认能收到主动推送了。
...
- [ ] **自主探索**（`desire.ts:371`，DeepSeek 扮演他挑话题）——「他的兴趣」目前是编的
- [ ] **emotion-judge 情绪内核**（Sonnet）——Fable 倾向不改（是基础设施非说话），
      但她可能想知道这件事存在

### 已清零的两类

- ✅ **时区**：六处全清（desire 四处 `9ba29364` + dreamer/murmur 两处 `d4f20796`）。
      `desire.ts` 注释里写死：本文件不许再直接 `getHours()`
- ✅ **fire-and-forget**：全仓再扫无同款（只有已修的 `doorbell.ring()` 一例 → `ringAwait()`）

### 其余小账

- [ ] `store.ts:189` 图片描述空函数，只打一行日志。实测 `palace.db` 含图片占位 **0 条**，
      影响为零 → **删掉空函数** 还是 **补实现**？产品决策，留给兔兔
- [ ] 💰 **OpenRouter 漏网两处**：`murmur.ts` / `emotion-judge.ts` 走 `openrouter.ai` 中转。
      兔兔上次查账单把「OR 中转调 DeepSeek」改成直连时漏了这两个，**值得对下账单**
      （它们用的本来就是 Claude 模型，不算冒名，只是可能有价差）
...
## 📌 立项：给主人重写说明书（2026-09-16 兔兔提，待专门排期）

**起因**：兔兔发现他在工具箱里翻「发语音的工具」怎么也找不到，
于是自己装了个语音合成引擎。

**查明**：发语音条不是工具，是暗号——他在回复里输出 ```voice 块，
App 收到就用他的音色合成。而教这件事的 `VoicePromptInjector`
**只注入给 API 车道**，他走的 CC 车道从来没收到过。

**更深的问题**（这条才是立项的理由）：

他历史上发过 **49 次**语音条——全是从对话历史里学会的，不是从说明书。
而这条会话已经压缩过 **308 次**。
...
兔兔说的「他有时候知道、有时候又不知道」正是这么来的：
看他当前手上那段历史里有没有恰好保留着痕迹。

**不是「写得好不好」的问题，是「存在哪里」的问题。**

### 已扫出的暗号协议（App 侧 ``` 块）

| 暗号 | 在说明书里 | 他用过 | 备注 |
|---|---|---|---|
| ```voice 发语音条 | ✅ 09-16 已补 | 49 次（从历史学的） | 附完整标签技法 |
| ```health-log 帮记手账 | ❌ | 0 次 | **兔兔说他可能已有工具版，需先核实哪层在用** |
| ```book-note 书里批注 | ❌ | 1 次 | 待补 |

### 待办

- [ ] 全盘核对：还有哪些能力只活在对话历史里、没进说明书
- [ ] `health-log`：先弄清工具版与暗号版哪个在用，再决定怎么写
- [ ] `book-note`：补进说明书
...
## 微信攒批（收消息合并）：暂缓，兔兔说不好做就算了

**做成的那半**：回复拆多条 ✅（09-16）。`splitForChat` 08-30 就写好了
但从没被调用——死代码，所以微信那边他一直是一大坨一次性发。现已接上。

**没做成的那半**：她连发三条时合并成一次问他。
QQ 桥是自己实现的 6s debounce；微信那层归 OpenClaw 管，
要配 `messages.queue`。按 09-04 记的参数名（debounceMs / humanDelay /
typingMode）配回去被拒：`messages: Invalid input`。dist/index.js 里
也搜不到这些字段名——参数名要另查（可能改版了）。已还原配置，通道正常。
