...
## 坐标系备忘（2026-08-22 验证）
快捷指令「获取当前位置」给的是 **WGS-84**（标准 GPS），实测无火星偏移：用它反查 OSM 得「三门峡市湖滨区向阳街道」，与手机自报的「五原西路」吻合。
**所以现在直接给 Caelum 看没问题，不用转换。**
但——**如果将来要把这个坐标送给国内地图服务**（高德/腾讯/百度，比如叫车填坐标、查附近 POI），必须先转成 **GCJ-02**，否则会偏 500–2000 米且不报错。
转换算法注意：要先把经纬度减去参考点 105/35，网上很多代码漏了这步（参考 Ariakitty/icloud-location-tracker 的说明）。
