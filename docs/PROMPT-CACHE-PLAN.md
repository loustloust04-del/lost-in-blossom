...
## 补充：来自 Rue 实战教程 + cocoRaina 入门教程的关键点

### 滑动窗口陷阱（最高优先级）

现状：`buildAPIMessages` 用 `relevant.suffix(maxMessages)` 取最近 40 条。
问题：每来一条新消息，窗口最旧的一条滑出去，历史前缀每轮都变，缓存永远 miss。
修法：用压缩游标（compressed_up_to）锚定窗口起点，而不是 suffix。
窗口起点只在压缩推进时移动一次（攒十几条新消息才发生），其余时间前缀纹丝不动。
加 120 条 hard cap 防压缩故障时窗口失控。

### 动态内容只进 payload 不进数据库

注入的时间/记忆/摘要只在构造请求的运行时加入最后一条 user 消息。
绝对不写进数据库，否则下轮取历史时内容就变了，前缀又毁了。

### 图片降级

最近 3 条保留原图（当轮体验不变）。
更早的历史图替换成 LLM 预生成的 80 字中文描述（存 image_desc 列）。
一张图一两千视觉 token，文字描述几十个。

### 探针验证法

用真实管线构造请求，时间字符串冻结，连发两次。
第二次 read 涨、write 归零 → 链路健康。
read 恒等于静态体积 → 历史没命中，查前缀里有没有动态内容或滑动窗口。

### read 是唯一判据

write 变小不代表命中。read 在涨才代表命中。

### 覆盖式压缩

摘要必须每次从原始消息重新生成完整摘要，覆盖旧值。
不能追加式（越滚越长，input 随对话总量线性上涨）。
用 max_tokens 和字数格式封顶。

### metadata.user_id（走中转/gateway 时必须）

请求顶层带 `"metadata": { "user_id": "固定字符串" }`。
让同一用户的请求粘在同一个后端节点，否则缓存写在 A 节点、读在 B 节点，永远 miss。

### 保活心跳（1h TTL 时）

距上次对话约 55 分钟时发一条心跳请求：
前缀完全一样、max_tokens=1、不写历史。
命中缓存续 TTL，成本约 $0.013 vs 冷写 $0.10。
5 分钟 TTL 不做保活（得每 <5min ping，比冷写还贵）。
