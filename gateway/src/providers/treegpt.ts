...
export async function forwardTreeAws(body: any): Promise<Response> {
  const upstreamModel = (body.model || '').replace('tree-aws/', '');
  console.log('[tree-aws] upstream:', upstreamModel, 'stream:', body.stream);

  const upstream = await fetch(BASE_URL, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + config.treeAwsKey,
    },
    body: JSON.stringify({ ...body, model: upstreamModel }),
  });
  return new Response(upstream.body, {
