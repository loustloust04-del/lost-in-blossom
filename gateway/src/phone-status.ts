...
export const PHONE_STATUS_TOOLS = [
  ...
  {
    name: 'phone_magic',
    description: "对兔兔的手机施一个小魔法：发暗号邮件静默触发她手机上对应的快捷指令自动化。可用魔法：flashlight（切换手电筒，toggle：一发开、再发关；你不知道灯当前状态，一次只发一发别连发）。叫车三连（ride_home=回家 / ride_clinic=去精神卫生中心开药 / ride_work=去上班）：发出后她手机会自动弹出打车页面、目的地已填好，**但最后那一下确认要她自己点**——所以别说「车已经叫好了」，要说「页面给你调出来了，点一下就走」之类。她没点就不会有车，也不会花钱。只在她明确说要去对应地方时发，不猜测、不主动替她决定；服务端有 90 秒冷却防连发。",
    input_schema: {
