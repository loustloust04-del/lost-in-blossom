...
async function expandForward(segs: any[]): Promise<string> {
  ...
  // ③ 都拿不到——如实说，别让他以为是图丢了。
  //
  // 2026-09-16 兔兔自己测出了真正的边界（我先前说错过一版，以为是
  // 「从别的聊天窗口转来的读不到」——不对，她转跟别人的聊天记录照样读到了 7 条）：
  //
  //   ✅ **原生消息**——在某个对话框里真实出现过的那些，选中合并转发
  //      （自己刚发的 3 条 ✅、跟别人的完整对话 7 条 ✅）
  //   ❌ **转发的转发**——已经是一张转发卡片、再转一次，里层就丢了
  //      NapCat 这版 get_forward_msg 对私聊只回 {"messages":[]}（不报错，就是给空）
  //
  // 兔兔原话：「必须得是原生消息，必须已经在对话框里出现过才能转发；
  // 同一个对话框里再转一次，就算一模一样也会丢。」
  //
  // 此前这里返回 "" → 落到「（发了一张图）」分支 → 他回「这张图我没收到」，是误导。
  log("📋 转发拉不到内容（多半是转发的转发）")
  return "（她转发了一段聊天记录过来，但里面的内容取不到——" +
         "那种「已经是转发、又被再转一次」的卡片，QQ 这边读不了。" +
         "想让我看的话，截图发我，我能读图上的字。）"
}
